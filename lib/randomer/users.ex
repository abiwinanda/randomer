defmodule Randomer.Users do
  @moduledoc """
  The Users context.
  """

  import Ecto.Query, warn: false
  alias Randomer.Repo

  alias Randomer.Users.User

  @doc """
  Returns the list of users.

  ## Examples

      iex> list_users()
      [%User{}, ...]

  """
  def list_users do
    Repo.all(User)
  end

  @doc """
  Gets a single user.

  Raises `Ecto.NoResultsError` if the User does not exist.

  ## Examples

      iex> get_user!(123)
      %User{}

      iex> get_user!(456)
      ** (Ecto.NoResultsError)

  """
  def get_user!(id), do: Repo.get!(User, id)

  @doc """
  Get at max two users with points greater than `max_number`

  ## Examples

      iex> Users.get_two_users_with_points_greater_than(60)
      [
        %Randomer.Users.User{
          __meta__: #Ecto.Schema.Metadata<:loaded, "users">,
          id: 5549110,
          inserted_at: ~U[2022-01-04 10:15:22Z],
          points: 75,
          updated_at: ~U[2022-01-04 10:55:29Z]
        },
        %Randomer.Users.User{
          __meta__: #Ecto.Schema.Metadata<:loaded, "users">,
          id: 5549167,
          inserted_at: ~U[2022-01-04 10:15:22Z],
          points: 87,
          updated_at: ~U[2022-01-04 10:55:29Z]
        }
      ]
  """
  def get_two_users_with_points_greater_than(max_number) do
    query =
      from u in User,
        where: u.points > ^max_number,
        limit: 2

    Repo.all(query)
  end

  @doc """
  Creates a user.

  ## Examples

      iex> create_user(%{field: value})
      {:ok, %User{}}

      iex> create_user(%{field: bad_value})
      {:error, %Ecto.Changeset{}}

  """
  def create_user(attrs \\ %{}) do
    %User{}
    |> User.changeset(attrs)
    |> Repo.insert()
  end

  @doc """
  Updates a user.

  ## Examples

      iex> update_user(user, %{field: new_value})
      {:ok, %User{}}

      iex> update_user(user, %{field: bad_value})
      {:error, %Ecto.Changeset{}}

  """
  def update_user(%User{} = user, attrs) do
    user
    |> User.changeset(attrs)
    |> Repo.update()
  end

  @doc """
  Updates all existing users points

  ## Examples

    iex> update_all_users_points()
    {1000000, nil}

  """
  def update_all_users_points() do
    query =
      from u in User,
        update: [set: [points: fragment("floor(random()*100)"), updated_at: ^DateTime.utc_now()]]

    Repo.update_all(query, [])
  end

  @doc """
  Deletes a user.

  ## Examples

      iex> delete_user(user)
      {:ok, %User{}}

      iex> delete_user(user)
      {:error, %Ecto.Changeset{}}

  """
  def delete_user(%User{} = user) do
    Repo.delete(user)
  end

  @doc """
  Returns an `%Ecto.Changeset{}` for tracking user changes.

  ## Examples

      iex> change_user(user)
      %Ecto.Changeset{data: %User{}}

  """
  def change_user(%User{} = user, attrs \\ %{}) do
    User.changeset(user, attrs)
  end
end
